...
const row = (tvdbId: number, season: number, episode: number, watchedAt = '2026-08-01T10:00:00.000Z'): TraktWatchRow => ({
  tvdbId,
  season,
  episode,
  watchedAt,
...
describe('traktRowsToApply', () => {
  it('writes a genuinely new episode of a show you track', () => {
    expect(traktRowsToApply([row(121361, 1, 2)], LIBRARY)).toHaveLength(1);
  });
  ...
  it('refuses an episode already watched — the duplicate is the whole danger', () => {
     ...
     * ordinary.
     */
    expect(traktRowsToApply([row(121361, 1, 1)], LIBRARY)).toEqual([]);
  });
  ...
  it('refuses a show you do not track rather than inventing a library', () => {
    ...
    expect(traktRowsToApply([row(999999, 1, 1)], LIBRARY)).toEqual([]);
  });
  ...
  it('refuses season 0', () => {
    ...
    expect(traktRowsToApply([row(121361, 0, 1)], LIBRARY)).toEqual([]);
  });
  ...
  it('lets the rest of a batch through when one row is refused', () => {
    ...
    const out = traktRowsToApply([row(999999, 1, 1), row(121361, 2, 5), row(121361, 0, 3)], LIBRARY);
    expect(out.map((r) => `${r.tvdbId}:${r.season}:${r.episode}`)).toEqual(['121361:2:5']);
  });

  it('is empty for an empty history rather than throwing', () => {
    expect(traktRowsToApply([], LIBRARY)).toEqual([]);
  });
});

describe('nextTraktWatermark', () => {
  it('is the newest watched_at actually seen', () => {
    const out = nextTraktWatermark(
  ...
  it('never moves backwards, even when Trakt backfills an older watch', () => {
     ...
     * every row in it.
     */
    expect(nextTraktWatermark([row(1, 1, 1, '2019-01-01T00:00:00.000Z')], '2026-08-01T00:00:00.000Z')).toBe(
  ...
  it('keeps the old mark when nothing arrived', () => {
    ...
    expect(nextTraktWatermark([], '2026-08-01T00:00:00.000Z')).toBe('2026-08-01T00:00:00.000Z');
  });

  it('stays null on a first run that found nothing', () => {
    expect(nextTraktWatermark([], null)).toBeNull();
  });
});

describe('traktWatchKey', () => {
  /*
  ...
  it('is what the decision compares on', () => {
    expect(traktWatchKey(121361, 1, 1)).toBe('121361:1:1');
  });

  it('matches the key a refusal is keyed on', () => {
    const library = { tracked: new Set([121361]), watched: new Set([traktWatchKey(121361, 1, 1)]) };
    expect(traktRowsToApply([row(121361, 1, 1)], library)).toEqual([]);
  });

  it('does not collide across shows, seasons or episodes', () => {
    const keys = [traktWatchKey(1, 11, 1), traktWatchKey(1, 1, 11), traktWatchKey(11, 1, 1)];
